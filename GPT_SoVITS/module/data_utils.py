...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, version=None, val=False):
        ...
        names4 = {name[:-3] for name in list(os.listdir(self.path4))}  # 去除.pt后缀
        names5 = set(os.listdir(self.path5))
        if self.is_v2Pro:
            names6 = {name[:-3] for name in list(os.listdir(self.path7))}  # 去除.pt后缀
        self.phoneme_data = {}
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if self.is_v2Pro:
                    sv_emb = torch.load(
                        f"{self.path7}/{audiopath}.pt", map_location="cpu"
                    )
        except:
            traceback.print_exc()
