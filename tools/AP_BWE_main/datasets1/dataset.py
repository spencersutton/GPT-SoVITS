...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._cache_ref_count == 0:
            audio, orig_sampling_rate = torchaudio.load(
                os.path.join(self.wavs_dir, filename + ".wav")
            )
        ...
        if orig_sampling_rate == self.hr_sampling_rate:
            ...
            audio_hr = aF.resample(
                audio, orig_freq=orig_sampling_rate, new_freq=self.hr_sampling_rate
            )

        audio_lr = aF.resample(
            audio, orig_freq=orig_sampling_rate, new_freq=self.lr_sampling_rate
        )
        audio_lr = aF.resample(
            audio_lr, orig_freq=self.lr_sampling_rate, new_freq=self.hr_sampling_rate
        )
        ...
        if self.split:
            if audio_hr.size(1) >= self.segment_size:
                ...
                audio_hr = torch.nn.functional.pad(
                    audio_hr, (0, self.segment_size - audio_hr.size(1)), "constant"
                )
                audio_lr = torch.nn.functional.pad(
                    audio_lr, (0, self.segment_size - audio_lr.size(1)), "constant"
                )
